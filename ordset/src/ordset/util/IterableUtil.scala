...
protected[ordset] object IterableUtil {
  ...
  def iterableCompare[A](xs: IterableOnce[A], ys: IterableOnce[A])(implicit ev: Order[A]): Int =
  ...
  def iterableEq[A](xs: IterableOnce[A], ys: IterableOnce[A])(implicit ev: Eq[A]): Boolean =
  ...
  def iterableHash[A](xs: IterableOnce[A])(implicit ev: Hash[A]): Int =
    cats.kernel.instances.StaticMethods.orderedHash(xs)(ev)

  def iteratorHash[A](xs: Iterator[A])(implicit ev: Hash[A]): Int =
    cats.kernel.instances.StaticMethods.orderedHash(xs)(ev)

  def iterableShow[A](xs: IterableOnce[A])(implicit ev: Show[A]): String =
