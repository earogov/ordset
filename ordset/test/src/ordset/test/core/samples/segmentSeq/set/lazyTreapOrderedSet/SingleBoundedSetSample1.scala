...
import ordset.test.core.samples.segmentSeq.set.treapOrderedSet.{SingleBoundedSetSample1 => TreapSingleBoundedSetSample1}
...
class SingleBoundedSetSample1[D[X] <: Domain[X]](
  ...
  override val labels: Set[Label] = super.labels + longSeedLabel(seed) + SegmentSeqLabels.singleBoundedSeq
