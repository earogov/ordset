...
trait FactoryValidationBehaviors {
  ...
  private object continuousUnbounded {
    ...
    val setTestCases = List[TestCase[Bound.Upper[Double]]](
      FailureCase(
        ...
        "Invalid sequence of bounds {x <= 0.0, x < -1.0}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[Double, Domain.ContinuousBounded](List(0d`]`, 1d`)`, -1d`]`)),
        "Invalid sequence of bounds {x < 1.0, x <= -1.0}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[Double, Domain.ContinuousBounded](List(0d`]`, 1d`]`, 1d`)`)),
        "Invalid sequence of bounds {x <= 1.0, x < 1.0}: sequence must be monotonically increasing"
      ...
      List(
        ...
        List(
          FailureCase(
            ...
            "Invalid sequence of values {0, 0}: adjacent values must be non-equal"
          ),
          FailureCase(
            OrderedMapFactoryIterable.default[Double, Domain.ContinuousBounded, BigInt](
              List((0d`]`, 100), (10d`)`, 0), (100d`]`, 1), (ExtendedBound.AboveAll, 1))
            ),
            "Invalid sequence of values {1, 1}: adjacent values must be non-equal"
          )
        )
      ).flatten
  }
  ...
  object continuousBounded {
    ...
    val setTestCases = List[TestCase[Bound.Upper[Double]]](
      FailureCase(
        OrderedSetFactoryIterable.single[Double, Domain.ContinuousBounded](0d`]`),
        "Invalid bound x <= 0.0: out of domain bounds [x > 0.0, x <= 100.0]"
      ),
      FailureCase(
      ...
      FailureCase(
        ...
        "Invalid bound x < 101.0: out of domain bounds [x > 0.0, x <= 100.0]"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[Double, Domain.ContinuousBounded](List(1d`]`, 100d`)`, 10d`]`)),
        "Invalid sequence of bounds {x < 100.0, x <= 10.0}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[Double, Domain.ContinuousBounded](List(1d`]`, 10d`]`, 10d`]`)),
        "Invalid sequence of bounds {x <= 10.0, x <= 10.0}: sequence must be monotonically increasing"
      ),
      SuccessCase(
        OrderedSetFactoryIterable.default[Double, Domain.ContinuousBounded](List())
      ),
  ...
  object discreteUnbounded {
    ...
    val setTestCases = List[TestCase[Bound.Upper[BigInt]]](
      FailureCase(
        ...
        "Invalid sequence of bounds {x <= 0, x < -11}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[BigInt, Domain.DiscreteUnbounded](
          List(BigInt(0) `]`, BigInt(5) `)`, BigInt(-11) `]`)
        ),
        "Invalid sequence of bounds {x < 5, x <= -11}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[BigInt, Domain.DiscreteUnbounded](
          List(BigInt(0) `]`, BigInt(5) `)`, BigInt(5) `)`)
        ),
        "Invalid sequence of bounds {x < 5, x < 5}: sequence must be monotonically increasing"
      ),
      FailureCase(
        OrderedSetFactoryIterable.default[BigInt, Domain.DiscreteUnbounded](
          List(BigInt(0) `]`, BigInt(5) `]`, BigInt(5) `)`)
  ...
  object discreteBounded {
    ...
    val setTestCases = List[TestCase[Bound.Upper[Int]]](
      FailureCase(
        ...
        "Invalid bound x <= -1: out of domain bounds [x >= 0, x <= 100]"
      ),
      // This case is tricky one. On the one hand bound `x < 101` is equivalent to `x <= 100` according to discrete
      // order and must be valid for domain, because it's inside domain bounds. But on the other hand 100 is the last
      // element of bounded domain, and domain has no information about its successor. So bounded domain can't figure
      // out, that bounds are the same. In other words bounds `x <= 100` and `x < 101` are equal on discrete
      // unbounded domain, and are different on given bounded discrete domain.
      FailureCase(
        OrderedSetFactoryIterable.single[Int, Domain.DiscreteBounded](101`)`),
        "Invalid bound x < 101: out of domain bounds [x >= 0, x <= 100]"
      ),
      FailureCase(
